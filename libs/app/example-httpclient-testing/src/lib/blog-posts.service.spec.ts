...
import { EnvironmentService } from '../../../../services/src/lib/environment.service';
...
describe('BlogPostsService', () => {
  ...
  describe('requests', () => {
    ...
    it('should expectOne url and method', () => {
      ...
      service.getAll().subscribe();
      backend.expectOne({
        url: `https://rails-rest.herokuapp.com/posts`,
        method: 'GET'
      });
      backend.verify();
    });
    ...
    it('should expectOne request', () => {
      ...
      backend.expectOne((request: HttpRequest<any>) => {
        return request.url.match(/posts/) && request.method === 'GET';
      });
      backend.verify();
    });
    ...
    it('should expectOne request different style (using TestRequest)', () => {
      ...
      const call: TestRequest = backend.expectOne(
        `https://rails-rest.herokuapp.com/posts`
      );
    ...
    it('should match two requests', () => {
      ...
      expect(calls[0].request.url).toEqual(
        `https://rails-rest.herokuapp.com/posts`
      );
      expect(calls[1].request.url).toEqual(
    ...
    it('should match different requests', () => {
      ...
      const otherCalls = backend.match(request => {
        return (
          request.url == `https://rails-rest.herokuapp.com/posts/1.json` &&
          request.method === 'GET'
        );
      });
      const calls = backend.match(request => {
        return (
          request.url == `https://rails-rest.herokuapp.com/posts` &&
          request.method === 'GET'
        );
      });
      ...
      expect(otherCalls[0].request.url).toEqual(
        `https://rails-rest.herokuapp.com/posts/1.json`
      );
      backend.verify();
    });
      ...
      const calls = backend.match(request => {
        return (
          ...
          request.urlWithParams ==
            `https://rails-rest.herokuapp.com/posts?page=1` &&
          request.method === 'GET'
        );
      });
  ...
  describe('responses', () => {
    it('should getPosts', () => {
      ...
      service.getAll().subscribe(result => {
        ...
        expect(result[0].created_at).toEqual(
          new Date('2017-12-07T04:39:49.447Z')
        );
      });
      backend
        .expectOne(`https://rails-rest.herokuapp.com/posts`)
        .flush(response);
      backend.verify();
    });
    ...
    it('should handle server error', () => {
      ...
      backend
        .expectOne(`https://rails-rest.herokuapp.com/posts`)
        .flush({}, { status: 422, statusText: 'Boom' });
      backend.verify();
    });
    ...
    it('should update post', () => {
      ...
      const call = backend.expectOne(
        `https://rails-rest.herokuapp.com/posts/2.json`
      );
    ...
    it('should fire only once', () => {
      ...
      const call1: TestRequest = backend.expectOne(
        `https://rails-rest.herokuapp.com/posts/1.json`
      );
      const call2: TestRequest = backend.expectOne(
        `https://rails-rest.herokuapp.com/posts/2.json`
      );
      call2.flush({ id: 2 });
    ...
    it('should cancel previous request (switchMap)', () => {
      ...
      const call1 = backend.expectOne(
        `https://rails-rest.herokuapp.com/posts/1.json`
      );
      const call2 = backend.expectOne(
        `https://rails-rest.herokuapp.com/posts/2.json`
      );
      ...
      const call3 = backend.expectOne(
        `https://rails-rest.herokuapp.com/posts/3.json`
      );
