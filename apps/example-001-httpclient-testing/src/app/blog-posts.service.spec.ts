...
describe('BlogPostsService', () => {
  ...
  describe('requests', () => {
    ...
    it('should match different requests', () => {
      ...
      const otherCalls = backend.match(request => {
        return request.url == `https://rails-rest.herokuapp.com/posts/1.json` && request.method === 'GET';
      });
      const calls = backend.match(request => {
        return request.url == `https://rails-rest.herokuapp.com/posts` && request.method === 'GET';
      });
  ...
  describe('responses', () => {
    ...
    it('should handle network error', () => {
      ...
      backend.expectOne(`https://rails-rest.herokuapp.com/posts`).error(new ErrorEvent(''), {
        status: 422,
    ...
    it('should fire only once for real', () => {
      service.get(1).subscribe(response => {
        expect(response.id).toEqual(1);
      });
      const call: TestRequest = backend.expectOne(`https://rails-rest.herokuapp.com/posts/1.json`);
      call.flush({ id: 1 });
      expect(call.cancelled).toBeTruthy(); // http.get close after firing.
      backend.verify();
    });
    ...
    it('should fire only once for real', () => {
      ...
      expect(call1.cancelled).toBeFalsy(); // http.get close after firing.
      call1.flush({ id: 1 });
      expect(call1.cancelled).toBeTruthy(); // http.get close after firing.
      backend.verify();
    });
